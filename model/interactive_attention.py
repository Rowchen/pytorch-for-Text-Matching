...
class DA(nn.Module):
    def __init__(self,arg,sta_feat=None):
        ...
        self.fc = nn.Sequential(
            nn.Dropout(0.5),
            nn.Linear(arg.compare_dim*4+add_dim,arg.fc_hiddim),
            nn.ReLU(inplace=True),
            nn.Dropout(0.5),
    ...
    def forward(self,x):
        ...
        o4b=F.softmax(o3,dim=2)   #the attentnion of b for a ,the sum of dim 2 is 1       
        o4a=F.softmax(o3,dim=1).permute(0,2,1)   #the attention of a for b ,after permuting the sum of dim 2 is 1
        ...
        o5a=self.compare(tr.cat([o2a,o5c],dim=2)).permute(0,2,1)
        o5b=self.compare(tr.cat([o2b,o5d],dim=2)).permute(0,2,1)
